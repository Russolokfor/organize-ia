...
export function AppShell({ children }: { children: React.ReactNode }) {
    ...
    <>
      <div className="p-6 pb-2 flex flex-col gap-1">
        <Link href="/dashboard" className="flex items-center gap-3" onClick={() => setIsMobileMenuOpen(false)}>
          ...
          <div className="flex flex-col">
            <span className="font-bold tracking-tight text-lg text-foreground leading-tight">
              Organize.ia
            </span>
            <span className="text-[10px] text-muted-foreground font-medium">Foco & Produtividade</span>
          </div>
        </Link>
        
        {/* Mobile close button */}
        <button 
          className="md:hidden p-2 text-muted-foreground hover:text-foreground absolute right-4 top-4"
      ...
      <div className="p-4 mt-auto border-t border-border/50 bg-card/50">
        <div className="flex items-center justify-between group">
          <Link href="/settings" onClick={() => setIsMobileMenuOpen(false)} className="flex items-center gap-3 flex-1 px-2 py-2 rounded-lg hover:bg-muted/50 transition-colors">
            ...
            <div className="flex flex-col overflow-hidden">
               ...
               <span className="text-[10px] text-muted-foreground truncate">Ver perfil</span>
            </div>
          </Link>
          <button
            onClick={handleLogout}
            className="p-2 text-muted-foreground hover:text-danger hover:bg-danger/10 rounded-lg transition-colors ml-1"
            title="Sair da conta"
          >
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col md:flex-row">
      {/* Desktop Sidebar */}
      <aside className="hidden md:flex w-72 flex-col border-r border-border/50 bg-card/10 backdrop-blur-2xl z-20 sticky top-0 h-screen">
        <SidebarContent />
      </aside>

      {/* Mobile Topbar */}
      <header className="md:hidden flex items-center justify-between p-4 border-b border-border/50 bg-card/40 backdrop-blur-xl sticky top-0 z-30">
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 rounded-lg bg-primary/20 flex items-center justify-center text-primary">
            <div className="w-4 h-4 border-2 border-current rounded-full" />
          </div>
          <span className="font-semibold text-lg tracking-tight">Organize<span className="text-primary">.ia</span></span>
        </div>
        <button 
        ...
        {isMobileMenuOpen && (
          <>
            ...
            <motion.aside 
              ...
              className="fixed inset-y-0 left-0 w-3/4 max-w-sm bg-card border-r border-border z-50 flex flex-col md:hidden shadow-2xl"
            >
              <SidebarContent />
            </motion.aside>
          </>
